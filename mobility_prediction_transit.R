...
mcmc_mobility <- function(MCMCITS)
{
  ...
  move_data=mobility$transit_stations_percent_change_from_baseline#[which(mobility$dow==weekday)]
...
{ 
  dat_m=mobility$transit_stations_percent_change_from_baseline#[which(mobility$dow==weekday)]
  seed=987
  set.seed(seed)
  proj=list()
  proj$mobility = dat_m
  ...
  for(td in 1:(wdelta*7))
  {
    ...
    proj$mobility[T0+td] = -rpois(1,-mu[T0+td])#rnorm(1,mu[T0+td-1],3)
  }
  # proj$dow=rep(weekday,length(proj$mobility))
  proj$mobility[1:T0] = -rpois(T0,-mu_mobility(dat_m,pars[iteration,]))
  proj$mobility[is.na(proj$mobility)|proj$mobility>0]=0
...
{
  mob_col = c("date","dow","transit_stations_percent_change_from_baseline")
  ...
  proj_mobility$transit_stations_percent_change_from_baseline = projector_mobility(wdelta,iteration)$mobility
...
for(i in seq(sampleit))
{
  ...
  output$move[i,]=p_m$transit_stations_percent_change_from_baseline
}
...
save(mobility_projection,file=paste0('working/',dat$doy[length(dat$doy)],'/movement_projection_transit.rdata'))
