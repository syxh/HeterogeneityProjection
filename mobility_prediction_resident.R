...
mcmc_mobility <- function(MCMCITS)
{
  set.seed(126)
  # pars_m = list(b0=1,b1=30,sd=0.1,a=100,b=15,c=0.1)
  pars_m = list(b0=1,d1=30,sd=0.1,a1=100,b1=15,c1=0.1,
                d2=30,sd=0.1,a2=100,b2=10,c2=0.2)
  # pars_m = list(b0=0,b1=10,sd=0.1,a=-1000,b=15,c=0.1)
  move_data=mobility$residential_percent_change_from_baseline#transit_stations_percent_change_from_baseline[which(mobility$dow==weekday)]
  ...
  for(iteration in 1:MCMCITS)
  {
    ...
    {
      oldpars = pars_m;
      pars_m$b0 = rnorm(1,pars_m$b0,1)# ; pars = mh(oldpars,pars,dat)
      
      pars_m$a1 = rnorm(1,pars_m$a1,10)# ; pars = mh(oldpars,pars,dat)
      pars_m$b1 = rnorm(1,pars_m$b1,2)# ; pars = mh(oldpars,pars,dat)
      pars_m$c1 = rnorm(1,pars_m$c1,0.05)# ; pars = mh(oldpars,pars,dat)
      pars_m$d1 = rnorm(1,pars_m$d1,0.5)# ; pars = mh(oldpars,pars,dat)
...
{ 
  dat_m=mobility$residential_percent_change_from_baseline#[which(mobility$dow==weekday)]
  seed=987
  set.seed(seed)
  proj=list()
  proj$mobility = dat_m
...
{
  mob_col = c("date","dow",
              "residential_percent_change_from_baselin"
  ...
  proj_mobility = data.frame(date = proj_date,
                             dow = wday(proj_date),
                             residential_percent_change_from_baselin = 0
                             # transit_stations_percent_change_from_baseline = 0
                             )
  proj_mobility$residential_percent_change_from_baselin = projector_mobility(wdelta,iteration)$mobility
...
# real_mobility = read.csv("data/SG_0703.csv")
# real_mobility = real_mobility[,-c(1:6)]
# real_mobility$date <- as.Date(as.character(real_mobility$date),"%Y/%m/%d")
# real_mobility = rbind(mobility_extr,real_mobility)
...
for(i in seq(sampleit))
{
  ...
  output$move[i,]=p_m$residential_percent_change_from_baselin
}


# plot(mobility$residential_percent_change_from_baselin,type='l',col=2)
# lines(output$move[1000,],type='l')
mobility_projection = output
save(mobility_projection,file=paste0('working/',dat$doy[length(dat$doy)],'/movement_projection_resident.rdata'))
