...
mcmc_mobility <- function(MCMCITS)
{
  ...
  move_data=mobility$retail_and_recreation_percent_change_from_baseline#[which(mobility$dow==weekday)]
...
{ 
  dat_m=mobility$retail_and_recreation_percent_change_from_baseline#[which(mobility$dow==weekday)]
  seed=987
  set.seed(seed)
  proj=list()
  proj$mobility = dat_m
...
{
  mob_col = c("date","dow","retails_percent_change_from_baseline")
  ...
  proj_mobility = data.frame(date = proj_date,
                             dow = wday(proj_date),
                             retails_percent_change_from_baseline = 0)
  proj_mobility$retails_percent_change_from_baseline = projector_mobility(wdelta,iteration)$mobility
...
for(i in seq(sampleit))
{
  ...
  output$move[i,]=p_m$retails_percent_change_from_baseline
}
...
save(mobility_projection,file=paste0('working/',dat$doy[length(dat$doy)],'/movement_projection_retails.rdata'))
